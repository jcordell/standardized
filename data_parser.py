...
class Data:
    ...
    #todo add a way to perform 0 mean 1 sigma normalization
    ...
    def __calculate_mean_sigma(self):
        self.__mean_sigma = []
        maxes = list(self.__data[0])
        mins = list(self.__data[0])
        for line in self.__data:
        ...
        for i in range(len(self.__features)):
            if isinstance(maxes[i], str):
                maxes[i] = None
                mins[i] = None
        self.__mean_sigma.append(maxes)
        self.__mean_sigma.append(mins)
