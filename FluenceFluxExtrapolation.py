...
def execute(model, data, savepath, *args, **kwargs):
    ...
    for x in range(len(fluence_divisions)):
        ...
        if responseSquared:
            ...
            Ypredict = np.power(Ypredict,0.5)
            RMSE = np.sqrt(mean_squared_error(Ypredict, np.power(np.array(data.get_y_data()).ravel(),0.5)))

        matplotlib.rcParams.update({'font.size': 26})
        if responseSquared: ax[x].scatter(np.power(data.get_y_data(),0.5), Ypredict, color='black', s=10)
        else: ax[x].scatter(data.get_y_data(), Ypredict, color='black', s=10)
